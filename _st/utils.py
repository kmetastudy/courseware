        ...
        for type in unit["types"]:
            ...
            if type == "v":
                ...
                if len(elements) == 1:
                    ...
                    sol_video = []
                    # solution text
                    if "sol_text" in info and len(info["sol_text"]) > 0:
                    ...
                    # solution video
                    if "sol_video" in info and len(info["sol_video"]) > 0:
...
def get_testum_info(request):
    ...
    for unit in units:
        ...
        for type in unit["types"]:
            ...
            if type == "v":
                elements = mElementN.objects.filter(
                    id=uuid.UUID(content_id), type=2, invalid=False
                )
                if len(elements) == 1:
                    info = json.loads(elements[0].json_data)
                    contentunit_list.append(
                ...
                elements = mElementN.objects.filter(
                    id=uuid.UUID(content_id), type=1, invalid=False
                )
                if len(elements) == 1:
                    info = json.loads(elements[0].json_data)
                    contentunit_list.append(
