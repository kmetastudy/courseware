...
// ClassLessonPlayer 역시 mtvPlayerTestum 과 같이
...
import { mtmPlayerLessonQuestion } from "../../../st/lesson/mtm-player-lesson-question.js";
import { mtmPlayerLessonResultTable } from "../../../st/lesson/mtm-player-lesson-result-table.js";
import { mtmPlayerLessonTitle } from "../../../st/lesson/mtm-player-lesson-title.js";
import { mtmPlayerVideoContent } from "../../../st/lesson/mtm-player-video-content.js";

import { mtmStudySubmitAction } from "../../../st/study/mtm-study-submit-action.js";
import { mtmPlayerLessonProgress } from "../../../st/lesson/mtm-player-lesson-progress.js";
import { mtmStudyMessagePopup } from "../../../st/study/mtm-study-message-popup.js";
...
export class ClassLessonPlayer {
  constructor(options = {}) {
    this.id = "id-mtm-player-lesson-" + ClassLessonPlayer.id++;
    this.elThis = null;
  ...
  _getContentPairIndex(index) {
    ...
    console.log("ClassLessonPlayer > _getContentPairIndex : ", unitIndex, itemIndex);
  ...
  _checkProgressPoint() {
    ...
    console.log(
      "ClassLessonPlayer > _checkProgressPoint : total_item_num = ",
  ...
  _prepareProgress() {
    ...
    for (var i = 0; i < this.player.lesson_unit_new.length; i++) {
      for (var j = 0; j < this.player.lesson_unit_new[i].types.length; j++) {
        // if (j > ClassLessonPlayer.UnitMaxQNum) break;
    ...
    console.log("ClassLessonPlayer > _prepareProgress : ", this.player);
  ...
  _wrongStep() {
    ...
    // console.log('ClassLessonPlayer > _wrongStep 1: ',this.player.current_rc_list,rc,this.player.currentItemIndex);
    // var rc = 0;
    if (rc > 1) {
      // 오답하기/다음하기
  ...
  _continueStep() {
    // console.log('ClassLessonPlayer > _continueStep', this.player.currentUnitIndex, this.player.currentItemIndex);
    var contentPairIndex = this._getContentPairIndex(this.player.currentIndexTotal + 1);

    console.log("ClassLessonPlayer > _continueStep", contentPairIndex);
    // console.log('ClassLessonPlayer > _continueStep',this.player.currentIndexTotal,this.player.finalIndexTotal);
  ...
  _questionSubmitLesson() {
    ...
    if (this.player.lesson_unit_new[this.player.currentUnitIndex].types[this.player.currentItemIndex] == "q") {
      ...
      console.log("ClassLessonPlayer > _questionSubmitLesson : answer = ", answer, " , value =", value);
  ...
  _videoContinueLesson() {
    ...
    if (this.player.bSolutionPlay) {
      console.log("ClassLessonPlayer > _videoContinueLesson : this.player.bSolutionPlay");
    ...
    // 순차적인 Video Content Play 인가?
      ...
      console.log(
        "ClassLessonPlayer > _videoContinueLesson : this.player.finalIndexTotal == this.player.currentIndexTotal",
      );
      ...
      console.log(
        "ClassLessonPlayer > _videoContinueLesson :",
        this.player.finalIndexTotal,
        this.player.currentIndexTotal,
      );
  ...
  _startVideo() {
    console.log("ClassLessonPlayer > _startVideo : ", this.player);
  ...
  _startQuestion() {
    ...
    if (qr == "O") {
      // console.log('ClassLessonPlayer > _startQuestion qr == O');
      ...
      // console.log('ClassLessonPlayer > _startQuestion else');
  ...
  _playLessonContent() {
    console.log("ClassLessonPlayer > _playLessonContent :", this.player.currentUnitIndex, this.player.currentItemIndex);
    if (this.player.lesson_content_new[this.player.currentUnitIndex].length > 0) {
      // console.log('ClassLessonPlayer > _playLessonContent : ',this.player.lesson_unit[this.player.currentUnitIndex].content_list);
  ...
  /////////////////////////////// Handler ////////////////////////////////
  onOnePanelHandler() {
    console.log("ClassLessonPlayer > onOnePanelHandler");
    // this.elExitButton.style.display = '';
  }
  onTwoPanelHandler() {
    console.log("ClassLessonPlayer > onTwoPanelHandler");
    // this.elExitButton.style.display = 'none';
  }

  onVideoCompleted() {
    console.log("ClassLessonPlayer : onVideoCompleted");
    ...
    else console.log("ClassLessonPlayer : onVideoCompleted --- abnormal");
  }
  ...
  onCorrectWrongHandler() {
    // console.log('ClassLessonPlayer : onCorrectWrongHandler');
    this._playLessonContent();
  }
  ...
  onNextStepHandler() {
    console.log("ClassLessonPlayer > onNextStepHandler : ");
    this._continueStep();
  }
  onConfirmHandler() {
    console.log("ClassLessonPlayer > onConfirmHandler : ");
    // this._continueStep();
    this._showPlayer(false);
    this._showResult(true);
  }
  onEventSolutionHandler(eData) {
    console.log("ClassLessonPlayer > onEventSolutionHandler : ", eData);
  ...
  onUnseenVideoClickHandler(tX, tY, value) {
    if (this.options.modeStudent)
      // 학생 모드
      return;
    console.log("ClassLessonPlayer > onUnseenVideoClickHandler :", tX, tY, value);
    ...
    if (this.player.lesson_unit[tY].content_list[tX].video == "1") {
      ...
      console.log("ClassLessonPlayer > onUnseenVideoClickHandler : ", index);
    }
  }
  // Lesson 결과 Table 을 Click 했을때, 적절한 해당 기능을 한다.
  onCellClickHandler(tX, tY, value) {
    console.log("ClassLessonPlayer > onCellClickHandler : ", tX, tY, value);
  ...
  setPlayOptions(options) {
    ...
    console.log("ClassLessonPlayer.>.setPlayOptions :", this.player.lesson_result_new);
    // prepare Title
  ...
  startLesson() {
    ...
    if (this.player.finalProcessIndex == -1 && this.options.modeStudent) {
      // and student mode
      console.log("ClassLessonPlayer > startLesson (new Start): ", this.player.finalProcessIndex);
      // 테스트를 처음하는 것...
      ...
      console.log("ClassLessonPlayer > startLesson (old Start) : ", this.player.finalProcessIndex);
  ...
  async urlUpdateLessonResult() {
    ...
    try {
      ...
      this._aurlUpdateLessonResultInfo(response.data);
