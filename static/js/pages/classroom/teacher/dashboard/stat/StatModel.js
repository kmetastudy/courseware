...
export default class StatModel extends Model {
  ...
  getAllStatByResult() {
    ...
    studyResults.forEach((studyResult) => {
      let id = studyResult.id;
      const user = this.findUser(studyResult.id_student);
      const name = user.full_name;
      const {
        json_data: { property: results },
      } = studyResult;
      let { studentChapterStat, studentProgress, studentPoint } = this.creatStudentStat(results);
  ...
  getScheduledCourse() {
    // Level 1인지 2인지 구분해주는 데코레이터로 변경해야함
    const { course: courseFullData } = this.get();
    let prevLevel = "";
    let prevDate = "";
    let chapter = "";
    let lesson = 0;
    ...
    const course = courseFullData.json_data.lists;
