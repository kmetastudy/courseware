...
export var mtmTreeAssign = function(options)
{
    this.id = 'id-mtm-tree-assign-' + mtmTreeAssign.id++;
...
mtmTreeAssign.prototype._addInputDate = function() 
{
    var options = {};
    options.title = "시작일";
    options.width = "50%";
...
mtmTreeAssign.prototype._addTreeItem = function(item) 
{
    var listItem = new mtmListItem(item);
    this.elThis.appendChild(listItem.elThis);
}

mtmTreeAssign.prototype._setTreeList = function() {
...
mtmTreeAssign.prototype._init = function() {
    this.elThis = document.createElement('div');
    this.elThis.classList.add('mtm-tree-assign');

    this._addInputDate();
    // for test
    this.options.treeList = mtmTreeAssign.testData;
    this._setTreeList();
...
mtmTreeAssign.prototype._fixLowLevel = function() {
...
mtmTreeAssign.prototype._releaseLowLevel = function() {
    this.fixLowLevel = false;
    for(var i=0;i<this.itemList.length;i++)
    {
        if(this.itemList[i].level > 1)
...
mtmTreeAssign.prototype._findIndex = function(uuid) {
    var currentIdx = -1;
    // console.log('mtmTreeAssign > onRemoveHandler uuid:',uuid);
    for(var i=0;i<this.itemList.length;i++)
    {
...
mtmTreeAssign.prototype._freezeLevel = function(level) {
    // 
    var index = -1;
    for(var i=0;i<this.itemList.length;i++)
    {
...
mtmTreeAssign.prototype._releaseLevel = function(level) {
    for(var i=0;i<this.itemList.length;i++)
    {
        if(this.itemList[i].level == level)
        {
...
mtmTreeAssign.prototype._changeItem = function(level) {
    ...
    // console.log("mtmTreeAssign > _changeItem index : ",startIndex, " , 갯수 : ", endIndex - startIndex + 1);
    ...
    // console.log("mtmTreeAssign > _changeItem List : ", itemList);
    ...
    // console.log('mtmTreeAssign > _changeItem (itemList): ', findIndex , itemList );
    for(var i=0;i<itemList.length;i++)
    {
        this.itemList.splice(i+findIndex,0,itemList[i]);
    }

    console.log('mtmTreeAssign > _changeItem (this.itemList) : ', this.itemList);
...
mtmTreeAssign.prototype.dragStartHandler = function(_,ui)
{
    ...
    console.log('mtmTreeAssign > dragStartHandler : end');
}

mtmTreeAssign.prototype.dragSortHandler = function(_,ui)
{
    // console.log('mtmTreeAssign > dragSortHandler : start : ',ui);
    return;
    ...
    console.log('mtmTreeAssign > dragSortHandler : end');
    
}

mtmTreeAssign.prototype.dragChangeHandler = function(_,ui)
{
    // var index = ui.placeholder.index()-1;
    // if(this.originalIndex != index)
    //     console.log('mtmTreeAssign > dragChangeHandler : start : ', this.originalIndex, index);
    ...
    console.log('mtmTreeAssign > dragChangeHandler : end');
}

mtmTreeAssign.prototype.dragStopHandler = function(_,ui)
{
    // return;
    console.log('mtmTreeAssign > dragStopHandler : start : ',this.originalIndex, ui.item.index()-1);
    // console.log('mtmTreeAssign > dragStopHandler : start : ', ui.placeholder.index());
    // $(this.elThis).sortable("cancel");
    this.changeIndex = ui.item.index()-1;
    ...
    console.log('mtmTreeAssign > dragStopHandler : end');
    
}

mtmTreeAssign.prototype.beforeStop = function(event, ui) {
    // if(ui.placeholder.index() == 2) 
    console.log('mtmTreeAssign > beforeStop : ', ui.placeholder.index());
    // return false;
}

// click or select ??
mtmTreeAssign.prototype.onBranchClickHandler = function() {

}

// title ?
mtmTreeAssign.prototype.onBranchChangeTitleHandler = function() {

}

// order ?
mtmTreeAssign.prototype.onBranchChangeOrderHandler = function() {

}

mtmTreeAssign.prototype.onAddHandler = function(clItem, uuid,index, actionIndex) {
    var currentIdx = -1;
    // console.log('mtmTreeAssign > onAddHandler uuid:',uuid);
    for(var i=0;i<this.itemList.length;i++)
    {
        if(this.itemList[i].uuid ==  uuid)
        {
    ...
    if(currentIdx < 0)
        return;
    // console.log('mtmTreeAssign > onAddHandler currentIdx :',currentIdx);
    // if(clItem.level)
    var action = {};
    var eventHandler = {};
    eventHandler.add = this.onAddHandler.bind(this);
...
mtmTreeAssign.prototype.onRemoveHandler = function(clItem, uuid,index) {
    var currentIdx = -1;
    // console.log('mtmTreeAssign > onRemoveHandler uuid:',uuid);
    for(var i=0;i<this.itemList.length;i++)
    {
    ...
    // console.log('mtmTreeAssign > onRemoveHandler :', this.itemList, 
...
mtmTreeAssign.prototype.onClickHandler = function(clItem, uuid,index) {
