...
// .contents .branch {
    // line-height : 2.4; 
// }
export var mtmListItem = function(options) {
    this.id = 'mtm-list-item-' + mtmListItem.id++;
    this.options = options;
    // console.log('mtmListItem : ', this.options);
    // if(!this.options)
    this.elThis = null;
...
mtmListItem.id = 0;
mtmListItem.minType = 0;
mtmListItem.contents = [
    { type : 0 , kind : 'chapter',},
    { type : 1 , kind : 'branch', }, 
    { type : 11, kind : 'testum',},
    { type : 12, kind : 'lesson',},
    { type : 13, kind : 'examination',},
    
];

mtmListItem.prototype._renderLatex = function(el)
{
...
// add Type Icon
mtmListItem.prototype._addTypeIcon = function(type)
{
    var elTypeIcon = null;
    
    // Todo. Jstar : icon 관련 setting 을 options 에서 넣어주면 어떤가?
    if(type >= mtmListItem.minType) 
    {
        elTypeIcon = document.createElement('i');
        if(type == 11)  // 테스텀
            // <i class="fa-regular fa-file-lines"></i>
            elTypeIcon.classList.add("fa-solid", "fa-file-lines");
        else if(type == 12) // 레슨
            elTypeIcon.classList.add("fa-solid", "fa-file-video");
        else if(type == 13) // 시험
            // <i class="fa-solid fa-envelope-open-text"></i>
            elTypeIcon.classList.add("fa-solid", "fa-envelope-open-text");
    }

    return elTypeIcon;
}

mtmListItem.prototype._addInputText = function(type)
{
    ...
    if(type >= mtmListItem.minType)
    {
        if(type == 11)
            ...
            elInputText.setAttribute("placeholder","시험 제목을 입력하세요.");

        elInputText.setAttribute('data-type',type);
        elInputText.setAttribute('data-id',this.uuid);
    }
...
mtmListItem.prototype._addActionEdit = function()
{
...
// add Action Remove Icons
mtmListItem.prototype._addActionRemove = function()
{
...
mtmListItem.prototype._addActionAdd = function()
{
    ...
    if(this.options.addItems)
    {
        ...
        {
            var index = i;
            if(this.options.addIndices && this.options.addIndices[i])
                index = this.options.addIndices[i];
...
mtmListItem.prototype._addAction = function(actionList)
{
...
mtmListItem.prototype._addAssign = function () {
    var optionsToggle = {};
    ...
    this.clAssignToggle.setValue(true);
...
//  +-- elChildrenBus
// 
mtmListItem.prototype._init = function() {
    ...
    this.elBranch.classList.add("branch");
...
//////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////// Handler //////////////////////////////////
mtmListItem.prototype.onToggleHandler = function(checked) {
    // console.log('mtmListItem > onToggleHandler');
    if(this.options && this.options.eventHandler && this.options.eventHandler.toggle)
        this.options.eventHandler.toggle(this,this.uuid, this.localIndex , checked);
}

mtmListItem.prototype.onRemoveClickHandler = function(e,id) {
    // ;//console.log('mtmListItem > onRemoveClickHandler : ', id);
...
mtmListItem.prototype.onAddClickHandler = function(e,data) {

    console.log('mtmListItem > onAddClickHandler :', data);
    if(this.options.addItems)
    {
...
mtmListItem.prototype.onExpandClickHandler = function(e,data) {
    // console.log('mtmListItem > onExpandClickHandler : ', data);
...
mtmListItem.prototype.onValueKeydownHandler = function(e)
{
...
// blur
mtmListItem.prototype.onValueBlurHandler = function(e)
{
    // console.log(' mtmListItem > onValueBlurHandler');
...
mtmListItem.prototype.onEditClickHandler = function(e)
{
    // console.log(' mtmListItem > onEditClickHandler');
    // e.stopPropagation();
    // e.preventDefault();
    if(this.value)
...
//////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////// API ////////////////////////////////////
mtmListItem.prototype.show = function(bShow)
{
    if(bShow)
...
mtmListItem.prototype.setExpandable = function(bExpandable)
{
    if(bExpandable)
...
mtmListItem.prototype.expand = function(bExpand)
{
    if(bExpand)
...
mtmListItem.prototype.setLocalIndex = function(index)
{
    this.localIndex = index; 
}

mtmListItem.prototype.getLocalIndex = function()
{
    return this.localIndex; 
}

mtmListItem.prototype.setSortable = function(bSortable)
{
    // elItem.classList.add("state-disabled");
    if(bSortable)
    {
...
mtmListItem.prototype.timeoutSlideUp = function() {
    this.elThis.style.display = 'none';
...
mtmListItem.prototype.slideUp = function(duration)
{
...
mtmListItem.prototype.timeoutSlideDown = function() {
    this.elThis.style.removeProperty('height');
    this.elThis.style.removeProperty('overflow');
    this.elThis.style.removeProperty('transition-duration');
    this.elThis.style.removeProperty('transition-property');
...
mtmListItem.prototype.slideDown = function(duration) {
...
mtmListItem.prototype.slideToggle = function(duration) {
...
// Set Edit Action
mtmListItem.prototype.setAction = function(bAction) {
    if(bAction)
        this.elAction.style.display = '';
    else
        this.elAction.style.display = 'none';

    this.setSortable(bAction);
}

// Set Assign
mtmListItem.prototype.setAssign = function(bAssign) {
    if(!this.elAssign)
        return;
...
mtmListItem.prototype.setAssignToggle = function(bAssignToggle) {
    if(!this.elAssign)
        return;
    ...
    {
        this.elAssign.style.display = '';
        this.clAssignToggle.show(true);
        this.clAssignStartDate.show(false);
        this.clAssignEndDate.show(false);
    }
    else
    {
        this.elAssign.style.display = 'none';
...
mtmListItem.prototype.setToggle = function(bToggle)
{
    if(this.clAssignToggle)
    {
        this.clAssignToggle.setValue(bToggle);
    }
} 

mtmListItem.prototype.setSelect = function(bSelect) {
    if(bSelect)
        this.elBranch.classList.add('selected');
    else 
        this.elBranch.classList.remove('selected');
}
