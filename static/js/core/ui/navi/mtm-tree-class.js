...
export var mtmTreeClass = function(options)
{
    this.id = 'id-mtm-tree-class-' + mtmTreeClass.id++;
...
mtmTreeClass.prototype._addInputDate = function() 
{
    var options = {};
    options.title = "시작일";
    options.width = "50%";
...
mtmTreeClass.prototype._addTreeItem = function(item) 
{
    var listItem = new mtmListItem(item);
    this.elThis.appendChild(listItem.elThis);
}

mtmTreeClass.prototype._setTreeList = function() {
...
mtmTreeClass.prototype._init = function() {
    this.elThis = document.createElement('div');
    this.elThis.classList.add('mtm-tree-question');

    this._addInputDate();
    // for test
    this.options.treeList = mtmTreeClass.testData;
    this._setTreeList();
...
mtmTreeClass.prototype._fixLowLevel = function() {
...
mtmTreeClass.prototype._releaseLowLevel = function() {
    this.fixLowLevel = false;
    for(var i=0;i<this.itemList.length;i++)
    {
        if(this.itemList[i].level > 1)
...
mtmTreeClass.prototype._findIndex = function(uuid) {
    var currentIdx = -1;
    // console.log('mtmTreeClass > onRemoveHandler uuid:',uuid);
    for(var i=0;i<this.itemList.length;i++)
    {
...
mtmTreeClass.prototype._freezeLevel = function(level) {
    // 
    var index = -1;
    for(var i=0;i<this.itemList.length;i++)
    {
...
mtmTreeClass.prototype._releaseLevel = function(level) {
    for(var i=0;i<this.itemList.length;i++)
    {
        if(this.itemList[i].level == level)
        {
...
mtmTreeClass.prototype._changeItem = function(level) {
    ...
    // console.log("mtmTreeClass > _changeItem index : ",startIndex, " , 갯수 : ", endIndex - startIndex + 1);
    ...
    // console.log("mtmTreeClass > _changeItem List : ", itemList);
    ...
    // console.log('mtmTreeClass > _changeItem (itemList): ', findIndex , itemList );
    for(var i=0;i<itemList.length;i++)
    {
        this.itemList.splice(i+findIndex,0,itemList[i]);
    }

    console.log('mtmTreeClass > _changeItem (this.itemList) : ', this.itemList);
...
mtmTreeClass.prototype.dragStartHandler = function(_,ui)
{
    ...
    console.log('mtmTreeClass > dragStartHandler : start : ', this.originalLevel,
    ...
    console.log('mtmTreeClass > dragStartHandler : end');
}

mtmTreeClass.prototype.dragSortHandler = function(_,ui)
{
    // console.log('mtmTreeClass > dragSortHandler : start : ',ui);
    return;
    ...
    console.log('mtmTreeClass > dragSortHandler : end');
    
}

mtmTreeClass.prototype.dragChangeHandler = function(_,ui)
{
    // var index = ui.placeholder.index();
    // if(this.originalIndex != index)
    //     console.log('mtmTreeClass > dragChangeHandler : start : ', this.originalIndex, index);
    ...
    console.log('mtmTreeClass > dragChangeHandler : end');
}

mtmTreeClass.prototype.dragStopHandler = function(_,ui)
{
    // return;
    console.log('mtmTreeClass > dragStopHandler : start : ',this.originalIndex, ui.item.index());
    // console.log('mtmTreeClass > dragStopHandler : start : ', ui.placeholder.index());
    ...
    console.log('mtmTreeClass > dragStopHandler : end');
    
}

mtmTreeClass.prototype.beforeStop = function(event, ui) {
    // if(ui.placeholder.index() == 2) 
    console.log('mtmTreeClass > beforeStop : ', ui.placeholder.index());
    // return false;
}

// click or select ??
mtmTreeClass.prototype.onBranchClickHandler = function() {

}

// title ?
mtmTreeClass.prototype.onBranchChangeTitleHandler = function() {

}

// order ?
mtmTreeClass.prototype.onBranchChangeOrderHandler = function() {

}

mtmTreeClass.prototype.onAddHandler = function(clItem, uuid,index, actionIndex) {
    var currentIdx = -1;
    // console.log('mtmTreeClass > onAddHandler uuid:',uuid);
    for(var i=0;i<this.itemList.length;i++)
    {
        if(this.itemList[i].uuid ==  uuid)
        {
    ...
    if(currentIdx < 0)
        return;
    // console.log('mtmTreeClass > onAddHandler currentIdx :',currentIdx);
    // if(clItem.level)
    var action = {};
    var eventHandler = {};
    eventHandler.add = this.onAddHandler.bind(this);
...
mtmTreeClass.prototype.onRemoveHandler = function(clItem, uuid,index) {
    var currentIdx = -1;
    // console.log('mtmTreeClass > onRemoveHandler uuid:',uuid);
    for(var i=0;i<this.itemList.length;i++)
    {
    ...
    // console.log('mtmTreeClass > onRemoveHandler :', this.itemList, 
...
mtmTreeClass.prototype.onClickHandler = function(clItem, uuid,index) {
