    ...
    {'step':0,'tag':'div', 'class':'class-mtv-input-select-scroll','attr':{'style':'width:50%;'}},
        {'step':1,'tag':'div','class':'class-box',},
            ...
            {'step':0,'tag':'ul','class':'class-list',},
                // {'step':1,'tag':'li','class':'class-selected','text':'선택'},
...
mtvInputSelectScroll.testListItem = [
    {'step':0,'tag':'li','class':'class-selected','text':'선택'},
    {'step':0,'tag':'li','text':'항목1'},
    {'step':0,'tag':'li','text':'항목2'},
    {'step':0,'tag':'li','text':'항목3'},
    {'step':0,'tag':'li','text':'항목4'},
...
// mtvInputSelectScroll.prototype.closeAllSelect = function(elmnt)
// {
    ...
    if(itemList.length > 0)
    {
        ...
        for(var i=1;i<itemList.length;i++)
        {
            if(bSelectLast && (i==itemList.length-1))
                this.itemList.push({'step':0,'tag':'li','class':'class-selected','text':itemList[i],'attr':{'data-index':i}});
            else
...
mtvInputSelectScroll.prototype.initOptions = function(options){
    ...
    if(!this.options.color)
        this.options.color = 'class-mtv-select-scroll-color-0';
...
mtvInputSelectScroll.prototype.listOnHandler = function(){
    ...
    if($(this.elsObject.elThis).hasClass('on'))
        $(this.elsObject.elList).css('display', 'block');
    else
...
mtvInputSelectScroll.prototype.listSelectHandler = function(e){
    ...
    $(e.target).addClass('class-selected').siblings('li').removeClass('class-selected');
...
mtvInputSelectScroll.prototype.initEvents = function() {
    ...
    if(!this.bInitEvent)
        this.elsObject.elSelect.addEventListener('click',this.listOnHandler.bind(this));
    ...
    // });
    if(!this.bInitEvent)
        document.addEventListener('click',this.listOffHandler.bind(this));
...
mtvInputSelectScroll.prototype.create = function(){    
    ...
    this.elCompList = mtvElementBuilder.buildComponent(mtvInputSelectScroll.staticBody,true);

    // console.log('mtvInputSelectScroll > this.elCompList : ', this.elCompList);
    ...
    // console.log('mtvInputSelectScroll > this.elThis : ', this.elThis);
    // console.log('mtvInputSelectScroll > this.elSelect : ', this.elSelect);
    // console.log('mtvInputSelectScroll > this.elList : ', this.elList);
    
    this.initElements();
    // console.log('mtvInputSelectScroll > this.elsObject : ', this.elsObject);
    
    // Add Select List Items
    // if(this.options.items.length == 0)
    //     for(var i=0;i<mtvInputSelectScroll.testListItem.length;i++)
    //     {
    //         var el = mtvElementBuilder.createElement(mtvInputSelectScroll.testListItem[i]);
