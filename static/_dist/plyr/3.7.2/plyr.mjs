...
const controls = {
  ...
  formatTime(time = 0, inverted = false) {
    ...
    let forceHours = getHours(this.duration) > 0; // console.log('formatTime : ', this.config);

    if (this.config && this.config.timeShift && this.config.timeShift.enable) forceHours = getHours(this.config.timeShift.duration) > 0; // }
    ...
    if (event) {
      switch (event.type) {
        ...
        case 'seeked':
          ...
          // to {
          value = getPercentage(this.currentTime, this.duration);
          if (this.config.timeShift && this.config.timeShift.enable) value = getPercentage(this.currentTime - this.config.timeShift.startTime, this.config.timeShift.duration); //}
          // Set seek range value only if it's a 'natural' time event

          if (event.type === 'timeupdate') {
            // jstar add this
            if (this.config.timeShift && this.config.timeShift.enable) {
              // const forwardTime = parseInt(this.currentTime) + this.config.timeShift.startTime;
              // if(!this.config.timeShift.forward)
              {
                if (this.currentTime > this.config.timeShift.forwardTime) {
                  this.config.timeShift.forwardTime = this.currentTime; // console.log('timeupdate : ',this.config.timeShift.forwardTime , this.currentTime);
                }
              } // else
  ...
  updateRangeFill(target) {
    ...
    if (matches(range, this.config.selectors.inputs.seek)) {
      ...
      let currentTime = controls.formatTime(this.currentTime);
      let duration = controls.formatTime(this.duration);
      const format = i18n.get('seekLabel', this.config);

      if (this.config.timeShift && this.config.timeShift.enable) {
        time = this.currentTime - this.config.timeShift.startTime;
        currentTime = controls.formatTime(time);
        duration = controls.formatTime(this.config.timeShift.duration);
      }

      range.setAttribute('aria-valuenow', time);
  ...
  updateSeekTooltip(event) {
    ...
    if (this.config.timeShift && this.config.timeShift.enable) duration = this.config.timeShift.duration;

    if (!this.config.tooltips.seek || !is.element(this.elements.inputs.seek) || !is.element(this.elements.display.seekTooltip) || duration === 0) {
      return;
    } //}
  ...
  timeUpdate(event) {
    ...
    if (this.config.timeShift && this.config.timeShift.enable) {
      currentTime = this.currentTime - this.config.timeShift.startTime;
      duration = this.config.timeShift.duration;
    }

    controls.updateTimeDisplay.call(this, this.elements.display.currentTime, invert ? duration - currentTime : currentTime, invert); // }
  ...
  durationUpdate() {
    ...
    if (is.element(this.elements.inputs.seek)) {
      this.elements.inputs.seek.setAttribute('aria-valuemax', duration);
    } // }
    ...
    if (!hasDuration && this.config.displayDuration && this.paused) {
      ...
      controls.updateTimeDisplay.call(this, this.elements.display.currentTime, duration);
    } // If there's a duration element, update content


    if (hasDuration) {
      // controls.updateTimeDisplay.call(this, this.elements.display.duration, this.duration);
      controls.updateTimeDisplay.call(this, this.elements.display.duration, duration);
    }
  ...
  setMarkers() {
    ...
    let duration = this.duration;
    if (this.config.timeShift && this.config.timeShift.enable) duration = this.config.timeShift.duration;
    if (!duration || this.elements.markers) return; //}
    // Get valid points
...
const ui = {
  ...
  migrateStyles() {
    ...
    .filter(key => !is.empty(key) && is.string(key) && key.startsWith('--plyr')).forEach(key => {
      if (key == '--value') {
        console.log('ui > migrateStyles : --value');
...
class Listeners {
  constructor(_player) {
    ...
    _defineProperty$1(this, "media", () => {
      ...
      on.call(player, player.media, 'progress playing seeking seeked', event => {
        // return;
        // console.log('listeners > Check for buffer progress');
      ...
      on.call(player, player.media, 'waiting canplay seeked playing', event => {
        // return;
        // console.log('listeners > Loading state');
    ...
    _defineProperty$1(this, "proxy", (event, defaultHandler, customHandlerKey) => {
      // console.log('proxy : ', event ,defaultHandler, customHandlerKey);
      const {
        player
      } = this;
    ...
    _defineProperty$1(this, "controls", () => {
      ...
      //  jstar
      // elements.inputs.seek.addEventListener('click', () => )
      ...
      this.bind(elements.inputs.seek, 'mousedown mousemove', event => {
        ...
        // return;
        // return;
        // console.log('listeners > seeking 1 :');
      ...
      this.bind(elements.inputs.seek, 'mousedown mouseup keydown keyup touchstart touchend', event => {
        // return;
        // console.log('listeners > Pause while seeking :');
        if (player.config.timeShift && player.config.timeShift.enable) {
          return;
        }
      ...
      this.bind(elements.inputs.seek, inputEvent, event => {
        // console.log('listeners > bind > seek');
        // return;
        ...
        seek.removeAttribute('seek-value'); // jstar mod 
        // this {
        //  player.currentTime = (seekTo / seek.max) * player.duration;
        //  }
        // to {
        // progress bar 를 click 했을 때....
        // 여기가 그 문제의 seek click 이네....
        // 좀 더 완벽하게, Click 시 왔다 갔다 안하고, 
        // maxForward 시점까지 표시해주면 좋겠지...
        // console.log('Listener seek : ');

        if (player.config.timeShift && player.config.timeShift.enable) {
          // console.log('Listener seek : ', player.config.timeShift);
          const currentTime = seekTo / seek.max * player.config.timeShift.duration + player.config.timeShift.startTime;

          if (!player.config.timeShift.forward) {
            if (player.config.timeShift.forwardTime >= currentTime) {
              player.currentTime = currentTime; // console.log('Listener seek : ok', player.config.timeShift.forwardTime,currentTime);
            } else {
              player.currentTime = player.config.timeShift.forwardTime; // console.log('Listener seek : modified', player.config.timeShift.forwardTime,currentTime);
            }
          } else {
            console.log('forward OK....', currentTime);
            player.currentTime = currentTime;
          }
        } else player.currentTime = seekTo / seek.max * player.duration; // }
      ...
      this.bind(elements.progress, 'mousedown touchstart', event => {
        // return;
        console.log('lsteners > Show scrubbing preview');
        const {
          previewThumbnails
        } = player;
      ...
      if (browser.isWebkit) {
        Array.from(getElements.call(player, 'input[type="range"]')).forEach(element => {
          this.bind(element, 'input', event => {
            // Todo. Jstar 진짜/진짜 여기가 seek 하는 곳....
            // 헌데.. Progress Thumb 은 어디서 하지?
            // <input type="range" > 는 thumb 을 보이지 않게 하지 않는 한,
            // slide range Click 시.... browser 에서 자동으로 그린다. 
            // 이를 custom 하려면, thumb 을 display = none 으로 해서 한다.
            // console.log('listeners > input :');
            // event.preventDefault();
            // event.stopPropagation();
            // return;
  ...
  handleKey(event) {
    ...
    const seekByIncrement = increment => {
      console.log('seekByIncrement'); // Divide the max duration into 10th's and times by the number value
      // jstar mod {
      ...
      player.currentTime = player.duration / 10 * increment;
      if (player.config && player.config.timeShift && player.config.timeShift.enable) ; //}
...
const vimeo = {
  ...
  ready() {
    ...
    console.log('----------------------- viemo ---------------------------'); // iframe.setAttribute(
    //   'allow',
  ...
  setup() {
    ...
    if (is.object(window.YT) && is.function(window.YT.Player)) {
      ...
      window.onYouTubeIframeAPIReady = () => {
        ...
        // this.debug.warn('plyr : onYouTubeIframeAPIReady');
  ...
  ready() {
    ...
    player.embed = new window.YT.Player(player.media, {
      ...
      extend({}, {
        ...
        cc_lang_pref: player.config.captions.language // Tracking for stats
      ...
      events: {
        ...
        onPlaybackRateChange(event) {
          ...
          player.media.playbackRate = instance.getPlaybackRate(); //   player.media.playbackRate = player.embed.getPlaybackRate();
        ...
        onReady(event) {
          ...
          player.media.play = () => {
            // console.log('plyr > youtube > play');
            player.media.stopped = false;
            assurePlaybackState.call(player, true);
            instance.playVideo(); // player.embed.playVideo();
          };

          player.media.pause = () => {
            // console.log('plyr > youtube > pause');
            assurePlaybackState.call(player, false);
            instance.pauseVideo(); // player.embed.pauseVideo();
          };

          player.media.stop = () => {
            console.log('plyr > youtube > stop');
            player.media.stopped = true;
            instance.stopVideo(); // player.embed.stopVideo();
          };

          player.media.duration = instance.getDuration(); // player.media.duration = player.embed.getDuration();

          player.media.paused = true;
          player.tmpCount = 0; // Seeking
          ...
          Object.defineProperty(player.media, 'currentTime', {
            ...
            set(time) {
              player.tmpCount++; // jstar mod. 여기가 seeking 하는 곳....

              console.log('..... seeking bar .... ', time, player.config.timeShift.startTime, player.currentTime); // if(player.tmpCount > 2)
              //     return;

              if (player.config.timeShift && player.config.timeShift.enable) {
                // if(player.currentTime >=  player.config.timeShift.startTime)
                if (!player.config.timeShift.forward) {
                  if (player.currentTime >= player.config.timeShift.forwardTime) {
                    if (time > player.config.timeShift.startTime + 1 && time > player.currentTime + 1) {
                      console.log('..... seeking cancel .... ');
                      return;
                    }
                  }
                }
              ...
              console.log('..... seeking ok .... ', time); // If paused and never played, mute audio preventively (YouTube starts playing on seek if the video hasn't been played yet).
              ...
              triggerEvent.call(player, player.media, 'seeking'); // if(player.config.timeShift && player.config.timeShift.enable)
              // {}
              // Seek after events sent

              instance.seekTo(time); // player.embed.seekTo(time);
            }

          });
          console.log('-------------------- youtube --------------------'); // Playback speed

          Object.defineProperty(player.media, 'playbackRate', {
            get() {
              return instance.getPlaybackRate(); // return player.embed.getPlaybackRate();
            },

            set(input) {
              instance.setPlaybackRate(input); // player.embed.setPlaybackRate(input);
            }
          ...
          Object.defineProperty(player.media, 'volume', {
            ...
            set(input) {
              volume = input;
              instance.setVolume(volume * 100); // player.embed.setVolume(volume * 100);
          ...
          Object.defineProperty(player.media, 'muted', {
            ...
            set(input) {
              ...
              muted = toggle;
              instance[toggle ? 'mute' : 'unMute'](); //   if(toggle)
              //     player.embed.mute();
              //     else
              //         player.embed.unMute();
          ...
          Object.defineProperty(player.media, 'currentSrc', {
            get() {
              return instance.getVideoUrl(); // return player.embed.getVideoUrl();
            }
          ...
          const speeds = instance.getAvailablePlaybackRates(); // const speeds = player.embed.getAvailablePlaybackRates();
          ...
          player.timers.buffering = setInterval(() => {
            ...
            player.media.buffered = instance.getVideoLoadedFraction(); // player.media.buffered = player.embed.getVideoLoadedFraction();
            ...
            if (player.media.buffered === 1) {
              clearInterval(player.timers.buffering); // console.log('canplaythrough');
              // Trigger event
        ...
        onStateChange(event) {
          console.log('------------------ on State Change -------------------'); // Get the instance
          ...
          switch (event.data) {
            ...
            case 0:
              ...
              if (player.media.loop) {
                ...
                // console.log('plyr looping');
                instance.stopVideo();
                instance.playVideo(); // player.embed.stopVideo();
                // player.embed.playVideo();
              } else {
                console.log('youtube : ended ');

                if (player.config.timeShift && !player.config.timeShift.ended) {
                  player.config.timeShift.ended = true;
                  triggerEvent.call(player, player.media, 'ended');
                }
              }
            ...
            case 1:
              ...
              if (config.customControls && !player.config.autoplay && player.media.paused && !player.embed.hasPlayed) {
                console.log('plyr > yutube > playing : pause');
                player.media.pause();
              } else {
                console.log('plyr > yutube > playing : playing');
                assurePlaybackState.call(player, true); // if(player.config.timeShift && !player.config.timeShift.ended)
                // {
                //     player.config.timeShift.ended = true;
                // }

                if (player.config.timeShift) player.config.timeShift.ended = false;
                triggerEvent.call(player, player.media, 'playing'); // Poll to get playback progress

                player.timers.playing = setInterval(() => {
                  triggerEvent.call(player, player.media, 'timeupdate'); // jstar add {
                  // to pause video for shift time 

                  if (player.config.timeShift && player.config.timeShift.enable && !player.media.stopped) {
                    if (player.currentTime - player.config.timeShift.startTime >= player.config.timeShift.duration) {
                      instance.pauseVideo(); // player.embed.pauseVideo();

                      player.currentTime = player.config.timeShift.startTime + player.config.timeShift.duration;
                      console.log('youtube : ended : by TimeShift ');
                    ...
                    //     player.currentTime = player.config.timeShift.startTime;
                    // and other function


                    if (player.currentTime < player.config.timeShift.startTime && !player.media.stopped) {
                      console.log('plyr re-arrange currentTime');
                      player.currentTime = player.config.timeShift.startTime; //+player.currentTime;
                    }
                  } // }
                ...
                if (player.media.duration !== instance.getDuration()) {
                  // if (player.media.duration !== player.embed.getDuration()) {
                  player.media.duration = instance.getDuration(); // player.media.duration = player.embed.getDuration();
    ...
    console.log('new window.YT.Player', player.media);
  }
...
const media = {
  ...
  setup() {
    ...
    if (this.isHTML5) {
      ...
      var iframe10 = this.elements.container.querySelector('iframe');
      console.log('----------- iframe 10 --------------', iframe10);
      youtube.setup.call(this);
      var iframe11 = this.elements.container.querySelector('iframe');
      console.log('----------- iframe 11 --------------', iframe11);
...
class Plyr {
  constructor(target, options) {
    ...
    this.media = target;
    console.log('----------- Plyr Step #1 ----------------- : ', target); // String selector passed
    ...
    // timeShift: { enable : true, startTime : 0, endTime : -1, forward : false},

    if (this.config.timeShift && this.config.timeShift.enable) this.config.timeShift.duration = this.config.timeShift.endTime - this.config.timeShift.startTime; // Elements cache
    ...
    console.log('----------- Plyr Step #1 -----------------'); // Log config options and support
    ...
    let iframe = null;
    let url = null;
    console.log('=========== plyr ========== :', _type); // Different setup based on type

    switch (_type) {
      case 'div':
        // Find the frame
        iframe = this.media.querySelector('iframe');
        console.log('----------- iframe --------------', iframe); // <iframe> type
      ...
      case 'audio':
        ...
        console.log('=========== plyr ========== :', _type); // Get config from attributes
    ...
    this.supported = support.check(this.type, this.provider, this.config.playsinline); // iframe = this.elements.container.querySelector('iframe');
    // console.log('----------- iframe 4 --------------',iframe);
    ...
    this.listeners = new Listeners(this); // iframe = this.elements.container.querySelector('iframe');
    // console.log('----------- iframe 4 --------------',iframe);
    // Setup local storage for user settings

    this.storage = new Storage(this); // Store reference

    this.media.plyr = this; // iframe = this.elements.container.querySelector('iframe');
    // console.log('----------- iframe 4 --------------',iframe);
    // Wrap media
    ...
    if (!is.element(this.elements.container)) {
      ...
      wrap(this.media, this.elements.container); // iframe = this.elements.container.querySelector('iframe');
      // console.log('----------- iframe 4 --------------',iframe);
    }

    iframe = this.elements.container.querySelector('iframe');
    console.log('----------- iframe 3 --------------', iframe); // Migrate custom properties from media to container (so they work 😉)

    ui.migrateStyles.call(this);
    iframe = this.elements.container.querySelector('iframe');
    console.log('----------- iframe 4 --------------', iframe); // Add style hook

    ui.addStyleHook.call(this);
    var iframe5 = this.elements.container.querySelector('iframe');
    console.log('----------- iframe 5 --------------', iframe5); // Setup media

    media.setup.call(this);
    var iframe6 = this.elements.container.querySelector('iframe');
    console.log('----------- iframe 6 --------------', iframe6); // Listen for events if debugging
    ...
    var iframe7 = this.elements.container.querySelector('iframe');
    console.log('----------- iframe 7 --------------', iframe7); // Setup interface
  ...
  set currentTime(input) {
    ...
    // jstar add : 
    // if(this.config.timeShift && this.config.timeShift.enable)
    //     this.media.currentTime = inputIsValid ? this.config.timeShift.startTime + input : 0;
    // else
  ...
  set duration(input) {
    ...
    this.media.duration = inputIsValid ? Math.max(input, 0) : 0; // Logging

    this.debug.log(`Force duration to ${this.duration} seconds`);
  }
  /**
  ...
  } // jstar add this for set timeShift function


  set timeShift(time) {
    this.config.timeShift.startTime = time.startTime;
    this.config.timeShift.endTime = time.endTime;
    this.config.timeShift.forward = time.forward;
    this.config.timeShift.forwardTime = time.forwardTime;
    this.config.timeShift.duration = this.config.timeShift.endTime - this.config.timeShift.startTime;
  } // jstar add this for get timeShift function


  get timeShift() {
    return this.config.timeShift;
  }
  /**
